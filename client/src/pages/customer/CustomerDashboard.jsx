    ...
    <div>
      ...
      <div className="page">
        ...
        <div className="grid-4" style={{ marginBottom: '2rem' }}>
          ...
          <StatCard icon="🚚" label="Active Deliveries" value={stats.active} color="var(--accent)" />
          <StatCard icon="✅" label="Delivered" value={stats.delivered} color="var(--success)" />
          <StatCard icon="🛢" label="Cylinders Received" value={stats.cylinders} color="var(--warning)" />
        </div>

        {activeOrder && (
          <div className="card" style={{ marginBottom: '2rem', borderColor: 'rgba(34,211,238,0.3)', background: 'rgba(34,211,238,0.04)' }}>
            <div className="flex-between" style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div className="live-dot" />
                <span style={{ fontWeight: 600 }}>Active Delivery in Progress</span>
              </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              {activeOrder.status === 'out_for_delivery' && (
                <Link to={`/customer/track/${activeOrder.orderId}`} className="btn btn-primary btn-sm">📍 Track Live</Link>
              )}
              <Link to={`/customer/chat/${activeOrder.chatRoomId}`} className="btn btn-ghost btn-sm">💬 Chat Agent</Link>
            </div>
          </div>
        )}
        ...
        <div className="card">
          <div className="flex-between" style={{ marginBottom: '1rem' }}>
            ...
            <Link to="/customer/orders" style={{ color: 'var(--primary)', fontSize: '0.85rem' }}>View all →</Link>
          </div>
          {recent.length === 0
            ? <EmptyState icon="📦" title="No orders yet" message="Book your first cylinder to get started." action={<Link to="/customer/orders/new" className="btn btn-primary">Book Now</Link>} />
            : (
                  ...
                  <tbody>
                    {recent.map(o => (
                      <tr key={o._id}>
                        ...
                        <td>
                          {o.status === 'out_for_delivery' && <Link to={`/customer/track/${o.orderId}`} className="btn btn-ghost btn-sm">📍 Track</Link>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
