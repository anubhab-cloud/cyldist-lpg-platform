...
export default function CustomerOrders() {
  ...
  const load = () => {
    ...
    ordersAPI.list({ limit: 50 })
      ...
      .catch(() => toast('Error', 'Failed to load orders', 'error'))
  ...
  const handleCancel = async (orderId) => {
    ...
    try {
      ...
      toast('Error', err.response?.data?.message || 'Cannot cancel order', 'error');
  ...
  return (
    <div>
      <Topbar title="My Orders">
        <Link to="/customer/orders/new" className="btn btn-primary btn-sm">➕ New Order</Link>
      </Topbar>
      <div className="page">
          ...
          : (
            <div className="card">
              <div className="table-wrap">
                <table>
                  <thead>
                    <tr><th>Order ID</th><th>Date</th><th>Qty</th><th>Amount</th><th>Address</th><th>Status</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
                    {filtered.map(o => (
                      <tr key={o._id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            {o.status === 'out_for_delivery' && (
                              <Link to={`/customer/track/${o.orderId}`} className="btn btn-ghost btn-sm">📍</Link>
                            )}
                            {o.chatRoomId && ['assigned','out_for_delivery'].includes(o.status) && (
                              <Link to={`/customer/chat/${o.chatRoomId}`} className="btn btn-ghost btn-sm">💬</Link>
                            )}
                            {['created'].includes(o.status) && (
                              <button className="btn btn-danger btn-sm" disabled={cancelling === o.orderId}
