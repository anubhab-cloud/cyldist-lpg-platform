...
const COLORS = { created: '#94a3b8', assigned: '#818cf8', out_for_delivery: '#22d3ee', delivered: '#10b981', cancelled: '#ef4444' };
...
export default function AdminDashboard() {
  ...
  const trendData = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      revenue: dayOrders.filter(o => o.status === 'delivered').reduce((s, o) => s + (o.totalAmount || 0), 0),
    };
  });
    ...
    <div>
      ...
      <div className="page">
        ...
        <div className="grid-4" style={{ marginBottom: '2rem' }}>
          ...
          <StatCard icon="₹" label="Total Revenue" value={`₹${(stats.revenue / 1000).toFixed(1)}K`} color="var(--success)" />
          ...
          <div className="card">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={trendData}>
                ...
                <Tooltip />
                <Line type="monotone" dataKey="orders" stroke="#6366f1" strokeWidth={2} dot={{ fill: '#6366f1', strokeWidth: 0, r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {warehouses.filter(w => w.availableCylinders < w.lowStockThreshold).length > 0 && (
          <div className="alert alert-error" style={{ marginTop: '1.5rem' }}>
            ...
            {warehouses.filter(w => w.availableCylinders < w.lowStockThreshold).map(w => w.warehouseName).join(', ')} — needs restocking.
          </div>
        )}
